...
calls(Call,Info,States) when is_tuple(Call) ->
  calls([Call],Info,States);
calls(Calls,Info,States) when is_list(Calls) ->
  NewStates =
    lists:map
      (fun (State) ->
	   PreCalls = 
	     lists:filter(fun (Call) -> pre(Call,Info,State) end, Calls),
	   State#state{incoming=PreCalls}
       end, States),
  execute(Info,NewStates).
